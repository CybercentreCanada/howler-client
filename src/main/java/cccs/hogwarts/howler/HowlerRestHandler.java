...
import cccs.hogwarts.howler.exceptions.HowlerRequestException;
...
public class HowlerRestHandler {
    ...
    public static <T> T futureGet(CompletableFuture<T> future) throws HowlerException {
        try {
            ...
            throw new HowlerException(String.format("Error occurred while sending your request to the Howler api: %s", e.getMessage()), e);
        }
    }

    public <T> T request(String method, String path, Class<T> responseClass, Header[] headers, Object body, Map<String, String> parameters) throws HowlerException {
    ...
    public <T> CompletableFuture<T> requestAsync(String method, String path, Class<T> responseClass,
        ...
        return CompletableFuture.supplyAsync(() -> {
            try (CloseableHttpClient httpClient = HttpClients.custom().setRedirectStrategy(new LaxRedirectStrategy()).build()) {
                ...
                log.debug(String.format("Howler request for (%s, %s)", request.getMethod(), request.getURI().toString()));

                // ResponseHandler - take care of status codes and mapping response body to Object class
    ...
    public <T> CompletableFuture<T> getAsync(String path, Class<T> klass, Header[] headers, Map<String, String> parameters) {
