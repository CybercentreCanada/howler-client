...
def test_same_analytic_only(client: Client):
    assert client.hit.generate_hash(
        {"howler.analytic": "analytic1"}
    ) == client.hit.generate_hash({"howler.analytic": "analytic1"})


def test_different_analytic_only(client: Client):
    assert client.hit.generate_hash(
        {"howler.analytic": "analytic1"}
    ) != client.hit.generate_hash({"howler.analytic": "analytic2"})


def test_same_analytic_same_detection(client: Client):
    assert client.hit.generate_hash(
        {"howler.analytic": "analytic1", "howler.detection": "detection1"}
...
def test_same_analytic_different_detection(client: Client):
    assert client.hit.generate_hash(
        ...
        {"howler.analytic": "analytic1", "howler.detection": "detection2"}
    )
